    ...
    private void start() {
        try {
          ...
          //  DatagramSocket ds = new DatagramSocket(9999);
            System.out.println("主服务器端开启，等待从服务器客户端连接中...");
            // 循环监听
            while (true) {
                // 等待客户端进行连接
                ...
                System.out.println("客户端[" + client.getRemoteSocketAddress() + "]连接成功，当前在线从客户端" + clients.size() + "个");
    ...
    class MessageListener extends Thread {
        ...
        public void run() {
            try {
                ...
                //sendMsg(0, "[系统消息]：" + client.getRemoteSocketAddress() + "当前共有" + clients.size() + "人在聊天", client);
                sendMsg(0,"[主服务器]："+client.getRemoteSocketAddress()+"从服务器处于存活状态",client);
                String msg;
                ...
                while (heartbeatMap.get(client) != null) {
                    ...
                    if ("heartbeat".equals(msg)) {
                        ...
                        //log
                        System.out.println("[" + client.getRemoteSocketAddress() + "]已宕机，当前服务器" + clients.size() + "个");

                        // 发送广播
                        sendMsg(0, "[系统消息]：" + client.getRemoteSocketAddress() + "已宕机，" + clients.size() + "台从服务器在线", client);
                    } else {
